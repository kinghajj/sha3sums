...
//F16
void getF16(uint_8t x[2],uint_8t y[2], uint_8t a,uint_8t b, uint_8t c){
    y[0]=x[1]^a;
    y[1]=x[0]^b^sbox[x[1]^c];  
}
//F32
void getF32(uint_8t x[4],uint_8t y[4],uint_8t a1,uint_8t b1, uint_8t c1,uint_8t a2,uint_8t b2, uint_8t c2,\
			uint_8t a3,uint_8t b3, uint_8t c3, uint_16t alpha, uint_16t beta,uint_16t gama){
    uint_8t i,z32[2],fz32[2];
	B16 pa,pb,pg; 
	pa.bit16=alpha; 
	pb.bit16=beta;
	pg.bit16=gama;
    for (i=0;i<2;i++){
		//we have to reverse alpha, beta and gama because they are unions
        y[i]=x[i+2]^pa.b8[2-i-1];
        z32[i]=x[i+2]^pg.b8[2-i-1];
        }
    getF16(z32,fz32,a3,b3,c3); 
	getF16(fz32,z32,a2,b2,c2);
	getF16(z32,fz32,a1,b1,c1); 
    for (i=0;i<2;i++)
        y[i+2]=x[i]^pb.b8[2-i-1]^fz32[i];
}
//F64
void getF64(uint_8t x[8],uint_8t y[8],uint_8t a1,uint_8t b1, uint_8t c1,uint_8t a2,uint_8t b2, uint_8t c2,uint_8t a3,\
			uint_8t b3, uint_8t c3, uint_16t alpha, uint_16t beta,uint_16t gama, uint_32t A, uint_32t B, uint_32t C){
    uint_8t i,z64[4],fz64[4];
    B32 PA, PC, PB; 
	PA.bit32=A;
	PB.bit32=B;
	PC.bit32=C;
    for (i=0;i<4;i++){
		//we have to reverse A, B and C because they are unions
        y[i]=x[i+4]^PA.b8[4-i-1];
        z64[i]=x[i+4]^PC.b8[4-i-1];
        }
    getF32(z64,fz64,a1,b1,c1,a2,b2,c2,a3,b3,c3,alpha,beta,gama); 
    for (i=0;i<4;i++)
        y[i+4]=x[i]^PB.b8[4-i-1]^fz64[i];
}

//Quasigroup Q64 operation
void getQ64(uint_8t x[8], uint_8t y[8], uint_8t q[8],uint_8t a1,uint_8t b1, uint_8t c1,uint_8t a2,uint_8t b2, uint_8t c2,\
			uint_8t a3,uint_8t b3, uint_8t c3,uint_16t alpha, uint_16t beta,uint_16t gama, uint_32t A, uint_32t B, uint_32t C){
     uint_8t z[8],fz[8],i,zpom[8],fpom[8];
     for (i=0;i<8;i++)
        z[i]=x[i]^y[i];
	 //we reverse z because of the little endian, all functions for extended Feistel network work as for big endian
     for(i=0;i<8;i++) 
      zpom[i]=z[7-i];
     getF64(zpom,fpom,a1,b1,c1,a2,b2,c2,a3,b3,c3,alpha,beta,gama,A,B,C);
	 //we reverse fz to get result in the little endian
	 for(i=0;i<8;i++)
      fz[i]=fpom[7-i];
...
//AE transformation
void AE(uint_64t m, uint_64t aa1[16], uint_64t bb1[16],int k,uint_8t a1,uint_8t b1,uint_8t c1,uint_8t a2,uint_8t b2,\
		uint_8t c2,uint_8t a3,uint_8t b3,uint_8t c3,uint_16t alpha, uint_16t beta,uint_16t gama, uint_32t A, uint_32t B, uint_32t C)
{   int i;    B64 m1,p; 
    m1.bit64=m; 
	for (i=0;i<k;i++){	p.bit64=aa1[i]+m1.bit64; m1.bit64=aa1[i];  
	   getQ64(p.b8,m1.b8,m1.b8,a1,b1,c1,a2,b2,c2,a3,b3,c3,alpha,beta,gama,A,B,C);  bb1[i]= m1.bit64;	}}

//RAE_m transformation
void RAE(uint_64t m, uint_64t aa1[16], uint_64t bb1[16],int k,uint_8t a1,uint_8t b1,uint_8t c1,uint_8t a2,uint_8t b2,\
		 uint_8t c2,uint_8t a3,uint_8t b3,uint_8t c3,uint_16t alpha, uint_16t beta,uint_16t gama, uint_32t A, uint_32t B, uint_32t C)
{   int i;    B64 m1,p;	p.bit64=m; 
	for (i=k;i>=0;i--){	  p.bit64=p.bit64+aa1[i]; m1.bit64=aa1[i];   
	   getQ64(m1.b8,p.b8,p.b8,a1,b1,c1,a2,b2,c2,a3,b3,c3,alpha,beta,gama,A,B,C);   bb1[i]= p.bit64;}}
...
void LinTr16(uint_64t a[16])
{ 
	int i,j; uint_64t pom;  
	for (j=1;j<=16;j++){  
		pom=a[15] ^ a[9] ^ a[6]^a[3] ;
...
{   uint_32t    i = ((uint_32t)(state->count[0]>>3) & Nasha256_MASK);
    ...
    /* we now need to mask valid bites and add the padding which is */
...
void LinTr32(uint_64t a[32])
{ 
	int i,j; uint_64t pom;  
	for (j=1;j<=32;j++){
		pom=a[31] ^ a[24] ^ a[14]^a[6] ;
		for (i=31;i>0;i--) a[i]=a[i-1];  
		a[0]=pom;
	}
}
