...
#define MARACA_LEN     16    /* number of 8-byte values per block */

#define MARACA_BLOCKS  64    /* array of pointers to accumulators */
#define MARACA_ACCUM   47    /* actual accumulators */

typedef  struct hashState {
  u8 hash[MARACA_LEN];  /* the internal state */
  u8 key[MARACA_LEN];   /* the key */
  u8 buf[MARACA_LEN];   /* an aligned block of data */
  u8 abuf[MARACA_ACCUM][MARACA_LEN]; /* accumulator buffers */
  u8 *a[MARACA_BLOCKS];        /* accumulators */
