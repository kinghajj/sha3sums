 ...
 * optimized ANSI C code for 32-bit processors
 *
...
static void Aurora256CF(AURORA_CTX *hctx, const BitSequence *m, const AURORA_UINT32 mask);
static void Aurora512MF(AURORA_CTX *hctx, const AURORA_UINT32 mask);
static void Aurora512CF(AURORA_CTX *hctx, const BitSequence *m, const AURORA_UINT32 mask);
...
#define WRITE128BIT(_dst, _src0, _src1, _src2, _src3)\
{ WRITE32BIT((_dst) + 0,  (_src0));\
  WRITE32BIT((_dst) + 4,  (_src1));\
  WRITE32BIT((_dst) + 8,  (_src2));\
  WRITE32BIT((_dst) + 12, (_src3));\
}

/* ulong to uchar (8-word = 256-bit) */
#define WRITE256BIT(_dst, _src0, _src1, _src2, _src3, _src4, _src5, _src6, _src7)\
{ WRITE128BIT((_dst) + 0,  (_src0), (_src1), (_src2), (_src3)); \
  WRITE128BIT((_dst) + 16, (_src4), (_src5), (_src6), (_src7)); \
}
...
#define ROTR32(_x, _n) (((_x) >> (_n)) | ((_x) << (32 - (_n))))

#define F0_0(_x) aurora_sm00[(_x)]
#define F0_1(_x) ROTR32(aurora_sm00[(_x)], 8)
#define F0_2(_x) ROTR32(aurora_sm00[(_x)], 16)
#define F0_3(_x) ROTR32(aurora_sm00[(_x)], 24)

#define F1_0(_x) aurora_sm10[(_x)]
#define F1_1(_x) ROTR32(aurora_sm10[(_x)], 8)
#define F1_2(_x) ROTR32(aurora_sm10[(_x)], 16)
#define F1_3(_x) ROTR32(aurora_sm10[(_x)], 24)
...
static void Aurora256CF(AURORA_CTX *hctx, const BitSequence *m, const AURORA_UINT32 mask)
{
  ...
  for(i = 0; i < 4; i++){
    READ32BIT(ml[i],     m + (i * 8));
    READ32BIT(ml[i + 4], m + (i * 8 + 4));
    READ32BIT(mr[i],     m + (i * 8 + 32));
    READ32BIT(mr[i + 4], m + (i * 8 + 36));
  }
  ...
  AURORA256_2ROUNDS(1); /* R1,  R2  */
  AURORA256_2ROUNDS(2); /* R3,  R4  */
  AURORA256_2ROUNDS(3); /* R5,  R6  */
  AURORA256_2ROUNDS(4); /* R7,  R8  */
  AURORA256_2ROUNDS(5); /* R9,  R10 */
  AURORA256_2ROUNDS(6); /* R11, R12 */
  AURORA256_2ROUNDS(7); /* R13, R14 */
  AURORA256_2ROUNDS(8); /* R15, R16 */
#else /* _LOOPED */
  for(i = 1; i <= 8; i++){
    AURORA256_2ROUNDS(i);
  }
...
#ifndef _LOOPED /* = _LOOP_UNROLL */
  AURORA512_2ROUNDS(1); /* R1,  R2  */
  AURORA512_2ROUNDS(2); /* R3,  R4  */
  AURORA512_2ROUNDS(3); /* R5,  R6  */
  AURORA512_2ROUNDS(4); /* R7,  R8  */
  AURORA512_2ROUNDS(5); /* R9,  R10 */
  AURORA512_2ROUNDS(6); /* R11, R12 */
  AURORA512_2ROUNDS(7); /* R13, R14 */
  AURORA512_2ROUNDS(8); /* R15, R16 */
#else /* _LOOPED */
  for(i = 1; i <= 8; i++){
    AURORA512_2ROUNDS(i);
  }
